...
public static class GitHubUpdateChecker
{
    ...
    private const string UpdateAttemptFileName = "last-update-attempt.txt";
    ...
    public static bool WasUpdateAlreadyAttempted(GitHubReleaseInfo release)
    {
        var path = GetUpdateAttemptPath();
        return File.Exists(path)
            && string.Equals(File.ReadAllText(path), GetUpdateAttemptKey(release), StringComparison.Ordinal);
    }

    public static void MarkUpdateAttempt(GitHubReleaseInfo release)
    {
        var path = GetUpdateAttemptPath();
        Directory.CreateDirectory(Path.GetDirectoryName(path));
        File.WriteAllText(path, GetUpdateAttemptKey(release));
    }
    ...
    public static async Task<string> DownloadInstallAndRestartAsync(GitHubReleaseInfo release)
    {
        ...
        var sourceExePath = NormalizeUpdateExeName(sourcePath);
        ...
        File.WriteAllText(scriptPath, CreateUpdateScript(Process.GetCurrentProcess().Id, sourcePath, appPath, launchPath, sourceExePath, updateRoot));
    ...
    private static string GetUpdateAttemptPath()
    {
        return Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
            "XGecuMetaCleaner",
            UpdateAttemptFileName);
    }

    private static string GetUpdateAttemptKey(GitHubReleaseInfo release)
    {
        return CurrentDisplayVersion + "|" + (release == null ? string.Empty : release.TagName);
    }
    ...
    private static string NormalizeUpdateExeName(string sourcePath)
    {
        var targetPath = Path.Combine(sourcePath, AppExeName);
        ...
        var currentExeName = Path.GetFileName(Assembly.GetExecutingAssembly().Location);
        var currentNamedPath = Path.Combine(sourcePath, currentExeName);
        if (File.Exists(currentNamedPath))
        {
            File.Copy(currentNamedPath, targetPath, true);
            return targetPath;
        }

        var exePath = Directory.GetFiles(sourcePath, "*.exe").FirstOrDefault();
    ...
    private static string CreateUpdateScript(int processId, string sourcePath, string appPath, string launchPath, string sourceExePath, string updateRoot)
    {
        return "@echo off\r\n"
            + "setlocal\r\n"
            + "set \"PID=" + processId + "\"\r\n"
            ...
            + "copy /Y \"" + sourceExePath + "\" \"" + launchPath + "\" >nul\r\n"
            + "if exist \"" + launchPath + "\" (\r\n"
            + "  start \"\" \"" + launchPath + "\"\r\n"
            + ")\r\n"
